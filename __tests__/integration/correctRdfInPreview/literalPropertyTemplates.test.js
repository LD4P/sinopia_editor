...
describe('RDF from literal property templates', () => {
  ...
  describe('repeatable, default value(s)', () => {
    ...
    describe('three values, 2 defaults', () => {
      ...
      it('defaultURI only', async () => {
        expect.assertions(9)
        await pupExpect(page).toClick('a[href="/editor"]', { text: 'test literal, repeatable, required, two defaults URI only (no literal)' })
        await pupExpect(page).toFill('input[placeholder=\'literal, repeatable, required, two defaults URI only\']', 'another')
        await page.keyboard.press('Enter')
        await expect(page).toClick('button', { text: 'Preview RDF' })
        const previewRdf = await page.$eval('pre', e => e.textContent)
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> <http://id.loc.gov/authorities/subjects/sh85027699> .')
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> <http://id.loc.gov/vocabulary/millus/fac> .')
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> "another"@en .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://examples.org/bogusOntologies/Resource> .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/ns/prov#wasGeneratedBy> "Sinopia:RT:Fixture:LiteralRepeat2DefaultsUriOnly" .')
        expect(numLines(previewRdf)).toEqual(5)
      })
      it('defaultURI and defaultLiteral', async () => {
        expect.assertions(9)
        await pupExpect(page).toClick('a[href="/editor"]', { text: 'test literal, repeatable, required, two defaultLiteral and defaultURI' })
        await pupExpect(page).toFill('input[placeholder=\'literal, repeatable, required, two defaults both\']', 'another')
        await page.keyboard.press('Enter')
        await expect(page).toClick('button', { text: 'Preview RDF' })
        const previewRdf = await page.$eval('pre', e => e.textContent)
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> <http://id.loc.gov/authorities/subjects/sh85027699> .')
        ...
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> "another"@en .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://examples.org/bogusOntologies/Resource> .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/ns/prov#wasGeneratedBy> "Sinopia:RT:Fixture:LiteralRepeat2DefaultsBoth" .')
        expect(numLines(previewRdf)).toEqual(5)
      })
      it('mixed', async () => {
        expect.assertions(9)
        await pupExpect(page).toClick('a[href="/editor"]', { text: 'test literal, repeatable, required, two defaults mixed' })
        await pupExpect(page).toFill('input[placeholder=\'literal, repeatable, required, two defaults mixed\']', 'another')
        await page.keyboard.press('Enter')
        await expect(page).toClick('button', { text: 'Preview RDF' })
        const previewRdf = await page.$eval('pre', e => e.textContent)
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> "mydefaultvalue"@en .')
        ...
        expect(previewRdf).toMatch('<> <http://examples.org/bogusOntologies/literal> "another"@en .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://examples.org/bogusOntologies/Resource> .')
        expect(previewRdf).toMatch('<> <http://www.w3.org/ns/prov#wasGeneratedBy> "Sinopia:RT:Fixture:LiteralRepeat2DefaultsMixed" .')
        expect(numLines(previewRdf)).toEqual(5)
      })
    })
  })
