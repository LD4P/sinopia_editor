...
beforeEach(() => {
  lookup.getLookupResult = jest.fn().mockResolvedValueOnce(result1)
...
describe('selecting a value from lookup', () => {
  ...
  it('allows manually entering a uri', async () => {
    ...
    // Value added
    await screen.findByText('http://example.com/', { selector: 'div.lookup-value span' })
  ...
  it('allows selecting a lookup', async () => {
    ...
    // Value added
    await screen.findByText('Shania Twain', { selector: 'div.lookup-value span' })
