...
describe('addTemplateHistory', () => {
  ...
  it('adds items uniquely', () => {
    let state = addTemplateHistory(createState().history, { payload: { key: 'template1' } })
  ...
  it('limits to 10', () => {
    const state = createState()
    state.history.templates = [{ id: 'template1' }, { id: 'template2' }, { id: 'template3' },
      { id: 'template4' }, { id: 'template5' }, { id: 'template6' }, { id: 'template7' },
      { id: 'template8' }, { id: 'template9' }, { id: 'template10' },
    ]

    const newState = addTemplateHistory(state.history, { payload: { key: 'template11' } })

    expect(newState.templates.map((template) => template.id)).toEqual(['template11', 'template1',
  ...
  it('does not add root resource template to history', () => {
    const state = createState()
    const newState = addTemplateHistory(state.history, { payload: { key: Config.rootResourceTemplateId } })
