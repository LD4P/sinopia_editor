...
export const addResourceFromDataset =
  (dataset, uri, resourceTemplateId, errorKey, asNewResource, otherResourceAttrs = {}) =>
  (dispatch) => {
...
const expandProperty = (property, errorKey) => (dispatch) => {
  if (property.propertyTemplate.type === "resource") {
    property.values = []
    const promises = property.propertyTemplate.valueSubjectTemplateKeys.map((resourceTemplateId) =>
      dispatch(newSubject(null, resourceTemplateId, {}, errorKey)).then((subject) =>
        dispatch(newPropertiesFromTemplates(subject, false, errorKey)).then((properties) => {
...
const recursiveOrderedObjects = (subjectTerm, objects, dataset, usedDataset) => {
  const firstQuad = dataset
    .match(subjectTerm, rdf.namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#first"))
    .toArray()[0]
  usedDataset.add(firstQuad)
  objects.push(firstQuad.object)
  const restQuad = dataset
    .match(subjectTerm, rdf.namedNode("http://www.w3.org/1999/02/22-rdf-syntax-ns#rest"))
    .toArray()[0]
...
const newUriFromObject = (obj, property, dataset, usedDataset) => {
  ...
  if (labelQuads.length > 0) {
    ...
    // This is to avoid user confusion over extra triples, e.g., https://github.com/LD4P/sinopia_editor/issues/2634
...
const newProperty = (subject, propertyTemplate, noDefaults, errorKey) => (dispatch) => {
  ...
  if (propertyTemplate.required && !property.values) {
    property.show = true
    return dispatch(valuesForExpandedProperty(property, noDefaults, errorKey)).then((values) => {
...
const valuesForExpandedProperty = (property, noDefaults, errorKey) => (dispatch) => {
  if (property.propertyTemplate.type === "resource") {
    return Promise.all(
      property.propertyTemplate.valueSubjectTemplateKeys.map((resourceTemplateId) =>
        dispatch(newSubject(null, resourceTemplateId, {}, errorKey)).then((subject) =>
          dispatch(newPropertiesFromTemplates(subject, noDefaults, errorKey)).then((properties) => {
            subject.properties = properties
            return newValueSubject(property, subject)
          })
        )
...
const newValueCopy = (valueKey, property) => (dispatch, getState) => {
  ...
  if (value.valueSubject) {
    return dispatch(newSubjectCopy(value.valueSubject.key, newValue)).then(() => newValue)
  }
