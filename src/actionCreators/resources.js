...
import { selectProperty, selectValue, selectFullSubject } from "selectors/resources"
import {
 ...
 */
export const loadResource = (uri, errorKey, asNewResource, preview) => (dispatch) => {
...
export const expandProperty = (propertyKey, errorKey) => (dispatch, getState) => {
  ...
  if (property.propertyTemplate.type === "resource") {
    promises = property.propertyTemplate.valueSubjectTemplateKeys.map((resourceTemplateId) =>
  ...
  return Promise.all(promises).then(() => dispatch(showProperty(property.key)))
}
