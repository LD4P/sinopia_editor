...
const validatePropertyTemplate = (template) => {
  ...
  if (template.type === "resource" && _.isEmpty(template.valueSubjectTemplateKeys)) {
    errors.push(
      `The field "${template.label}" with property "${template.uri}" has type nested resource, but does not ` +
        `specify a template in Nested resource attributes.`
    )
  }
  return errors
}
...
const validateAllUniqueResourceURIs = (propertyTemplates, resourceTemplatePromises) => (dispatch) =>
  Promise.all(
    propertyTemplates.map((propertyTemplate) =>
      dispatch(validateUniqueResourceURIs(propertyTemplate, resourceTemplatePromises))
    )
...
const validateUniqueResourceURIs = (propertyTemplate, resourceTemplatePromises) => (dispatch) => {
  ...
  return Promise.all(
    ...
    return multipleClasses.map((clazz) => {
      ...
      return (
        `The following resource templates references for ${propertyTemplate.uri} have the same class (${clazz}), ` +
        `but must be unique: ${classIdsStr}`
      )
    })
  })
}
