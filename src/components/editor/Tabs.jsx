...
const Tabs = (props) => {
    ...
    return (
      <li className="nav-item" role="presentation">
        <a
          ...
          href={href}
          role="tab"
          aria-controls={child.props.eventKey}
  ...
  const tabs = chunkedChildren.map((childPair) => (
    <div className="row" key={`${childPair[0].props.eventKey}-div`}>
      <div className="col-6">{renderTab(childPair[0])}</div>
      {childPair[1] && <div className="col-6">{renderTab(childPair[1])}</div>}
    </div>
  ))
