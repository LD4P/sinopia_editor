...
const RDFModal = (props) => {
  ...
  const modal = (
    <div
      className={useModalCss(props.show)}
      id="rdf-modal"
          ...
          <div className="modal-body rdf-modal-content">
            <div className="row" style={{ marginLeft: "0", marginRight: "0" }}>
              <div className="col-sm-6">
                If this looks good, then click Save
              </div>
              <div className="col-sm-6" style={{ textAlign: "right" }}>
                <SaveAndPublishButton class="modal-save" />
              </div>
            </div>
            {props.show && <RDFDisplay dataset={props.dataset()} />}
          </div>
        </div>
      </div>
    </div>
...
const mapStateToProps = (state) => ({
  ...
  dataset: () =>
    new GraphBuilder(selectFullSubject(state, selectCurrentResourceKey(state)))
      .graph,
})
