...
const displayProperty = (params) => {
  ...
  if (_.isEmpty(element)) return <div>{header}<h1>{key} not found</h1></div>
  return (
    <div>
      {header}
      <h1>{key}</h1>
      <h3><em>{element.url}</em></h3>
...
const AllProperties = () => (
  <div>
    <h1>Vocabulary</h1>
    <p>
      Sinopia uses a number of properties and classes for adding
      and editing RDF resources.
    </p>
    {
        ...
        return (
          <div className="card w-50 mb-2" id={key} key={key}>
            <div className="card-body">
              <h2 className="card-title"><a href={`/vocabulary/${key}`}>{key}</a></h2>
