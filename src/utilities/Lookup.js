...
export const getLookupResult = (query, authorityConfig, startOfRange) => {
  if (authorityConfig.uri.startsWith('urn:ld4p:sinopia')) return getSinopiaLookupResult(query, authorityConfig, { startOfRange })
  return getQALookupResult(query, authorityConfig, { startOfRange })
}

const getSinopiaLookupResult = (query, authorityConfig, options) => getSinopiaSearchLookupResult(query, authorityConfig, options)
  .then((result) => ({ ...result, authorityConfig }))

const getQALookupResult = (query, authorityConfig, options) => createLookupPromise(query, authorityConfig, options)
  .then((response) => {
